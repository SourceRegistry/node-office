...
// --- XML Parser (secure config) ---
    ...
    export class Page {
        ...
        constructor(public content: Content[] = [], public metadata: Record<string, any> = {}) {
        }
    ...
    export function parse(zip: ZipReader): Document {
    ...
    export function parseFromPath(path: string){
        return parse(ZipReader.fromPath(path));
    }

    export async function parseFromBlob(data: Blob){
        return parse(await ZipReader.fromBlob(data));
    }
...
// --- Text extraction (structured) ---
    function collectPptxTextStructured(obj: any, out: Content[]): void {
        if (obj == null) return;
